...
from _algo.utils import parse_txt
...
def draw_roc(_result: str, save_dir: str = './'):
    """
    Draw roc and compute auc for binary classification
    Args:
        _result:  result file. filename, probability, prediction, gt
        save_dir: str, where to save. default ./

    Returns: auc, int
    """
    _, probability, pred, gt = zip(*parse_txt(_result, convert2float_if_necessary=1))
...
def draw_confusion_matrix(_result: str, class_mapping: str = None, save_dir: str = './', num_classes: int = None):
    """
    ...
    """
    _, _, prediction, gt = zip(*parse_txt(_result, convert2float_if_necessary=1))
